...
do
  grep $j $3 | cut -d= -f1 
  sed '1d' $1 | sed 's/"//g' | awk -F, -v y="$j" '($5=="0") && ($14==y) {sum += $12} END {printf "%-10.0f\n", sum}'
done
...
do
  grep $i $3 | cut -d= -f1 
  sed '1d' $1 | sed 's/"//g' | awk -F, -v x="$i" '($5=="0") && ($13==x) {sum += $12} END {printf "%-10.0f\n", sum}'
done
...
do
  ...
  do
    grep $i $3 | cut -d= -f1
    grep $j $3 | cut -d= -f1
    sed '1d' $1 | sed 's/"//g' | awk -F, -v x="$i" -v y="$j" '($5=="0") && ($13==x) && ($14==y) {sum += $12} END {printf "%-10.0f\n", sum}'
  done
done 
# -------------------------------------------------------------
#
