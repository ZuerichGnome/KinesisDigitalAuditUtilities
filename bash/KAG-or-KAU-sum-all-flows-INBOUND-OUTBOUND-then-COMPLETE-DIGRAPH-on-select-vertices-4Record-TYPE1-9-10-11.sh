...
do
  grep $j $3 | cut -d= -f1 
  sed '1d' $1 | sed 's/"//g' | awk -F, -v y="$j" '($5=="1") && ($10==y) {sum += $11} END {printf "%-10.0f\n", sum}'
done
...
do
  grep $i $3 | cut -d= -f1 
  sed '1d' $1 | sed 's/"//g' | awk -F, -v x="$i" '($5=="1") && ($9==x) {sum += $11} END {printf "%-10.0f\n", sum}'
done
...
do
  ...
  do
    grep $i $3 | cut -d= -f1
    grep $j $3 | cut -d= -f1
    sed '1d' $1 | sed 's/"//g' | awk -F, -v x="$i" -v y="$j" '($5=="1") && ($9==x) && ($10==y) {sum += $11} END {printf "%-10.0f\n", sum}'
  done
done 
# -------------------------------------------------------------
#
